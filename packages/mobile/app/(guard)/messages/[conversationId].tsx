...
export default function GuardChatScreen() {
  ...
  const [replyTo, setReplyTo] = useState<{
    messageId: string; senderName: string; content: string | null;
  } | null>(null);
  ...
  const [actionSheet, setActionSheet] = useState<{
    visible: boolean; message: ChatMessage | null;
  ...
  useEffect(() => {
    if (messages.length > 0 && user?.id) {
      ...
      if (latestMsg && !latestMsg._optimistic) markRead(latestMsg.id);
    }
  }, [messages, user?.id, markRead]);

  // BUG-10 fix: Check both guards and residents for sender name
  const senderNameRef = useRef('');
  useEffect(() => {
    if (!user?.id) return;
    Promise.all([
      supabase.from('guards').select('first_name').eq('user_id', user.id).maybeSingle(),
      supabase.from('residents').select('first_name').eq('user_id', user.id).maybeSingle(),
    ]).then(([guardsRes, residentsRes]) => {
      senderNameRef.current =
        guardsRes.data?.first_name ?? residentsRes.data?.first_name ?? 'Guard';
    });
  }, [user?.id]);
  ...
  const handleLongPress = useCallback((message: ChatMessage) => {
    ...
    const actions: any[] = [
      {
        label: 'Reply', icon: 'arrow-undo-outline',
        onPress: () => setReplyTo({ messageId: message.id, senderName: message.sender_name ?? 'Unknown', content: message.content }),
      },
      {
        label: 'Copy', icon: 'copy-outline',
        onPress: () => { if (message.content) Clipboard.setStringAsync(message.content); },
      },
    ];
    if (isOwn) {
      actions.push({
        label: 'Edit', icon: 'pencil-outline',
        onPress: () => setEditingMessage({ messageId: message.id, content: message.content ?? '' }),
      });
      actions.push({
  ...
  const handleReaction = useCallback(
    (messageId: string, reaction: string) => { toggleReaction.mutate({ messageId, reaction }); },
    [toggleReaction]
  );

  const handleSubmitEdit = useCallback(
    (messageId: string, newContent: string) => { editMessage.mutate({ messageId, newContent }); setEditingMessage(null); },
    [editMessage]
  );
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: ChatMessage; index: number }) => {
      ...
      const showSender = isGroup && !isOwn && (!prevMsg || prevMsg.sender_id !== item.sender_id);
      ...
      return (
        <React.Fragment key={item.id}>
          <MessageBubble
            message={item} isOwn={isOwn} isRead={isRead} showSender={showSender}
            onLongPress={handleLongPress} onReaction={handleReaction}
            onMediaPress={setMediaPreviewUri} onReplyPress={() => {}}
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          ...
          {isGroup && conversationInfo && (
            <Text style={styles.headerSubtitle}>{conversationInfo.participant_count} members</Text>
          )}
        </View>
        <TouchableOpacity style={styles.moreButton} onPress={() => {
          if (!conversationInfo) return;
          const isMuted = conversationInfo.is_muted;
          showAlert(isMuted ? 'Unmute' : 'Mute', isMuted ? 'Re-enable notifications?' : 'Mute notifications?', [
            { text: 'Cancel', style: 'cancel' },
            { text: isMuted ? 'Unmute' : 'Mute', onPress: () => muteConversation.mutate({ conversationId: conversationId!, mute: !isMuted }) },
          ]);
        }}>
          <Ionicons name={conversationInfo?.is_muted ? 'notifications-off-outline' : 'ellipsis-vertical'} size={22} color={colors.textBody} />
        </TouchableOpacity>
      </View>
      ...
      <KeyboardAvoidingView style={styles.keyboardAvoid} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        {isLoading ? (
          ...
          <FlatList
            ...
            removeClippedSubviews={Platform.OS === 'android'} maxToRenderPerBatch={15}
            ListFooterComponent={isFetchingNextPage ? <ActivityIndicator size="small" color={colors.primary} style={{ paddingVertical: spacing.xl }} /> : null}
            ListEmptyComponent={
        ...
        <ChatInput
          onSend={handleSend} onAttach={handleAttach} onTyping={handleTyping}
          replyTo={replyTo} editingMessage={editingMessage}
          onCancelReply={() => setReplyTo(null)} onCancelEdit={() => setEditingMessage(null)}
          onSubmitEdit={handleSubmitEdit}
        />
      </KeyboardAvoidingView>

      <MediaPreview uri={mediaPreviewUri} visible={!!mediaPreviewUri} onClose={() => setMediaPreviewUri(null)} />
      <MessageActionSheet visible={actionSheet.visible} onClose={() => setActionSheet({ visible: false, message: null })} actions={actionSheetActionsRef.current} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { fontFamily: fonts.medium, fontSize: 12, color: colors.textCaption },
  ...
  emptyContainer: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: spacing.lg, transform: [{ scaleY: -1 }] },
  emptyText: { fontFamily: fonts.medium, fontSize: 15, color: colors.textMuted, transform: [{ scaleY: -1 }] },
});
