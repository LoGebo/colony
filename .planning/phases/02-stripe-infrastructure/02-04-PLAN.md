...
must_haves:
  truths:
    ...
    - "curl to create-payment-intent without JWT returns 401"
    - "curl POST to payment-webhook without stripe-signature header returns 400"
  artifacts: []
  key_links:
...
```
Expected: create-payment-intent returns 401, payment-webhook returns 400 or 405.
  </action>
  <verify>
...
- curl POST to payment-webhook returns 400 (missing signature header)
  </verify>
  <done>
Database migrations applied (3 tables live), both Edge Functions deployed and responding to HTTP requests. create-payment-intent returns 401 on unauthenticated request, payment-webhook returns 400 on request without stripe-signature header.
  </done>
</task>
