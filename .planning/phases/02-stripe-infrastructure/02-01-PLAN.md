...
- RLS is enabled and policies use correct role names ('admin', 'community_admin', 'manager')
  ...
  <done>
stripe_customers and webhook_events tables exist in the database with proper constraints, indexes, RLS policies, and comments.
  </done>
</task>
...
- Admin role check includes 'community_admin'
  ...
  <done>
payment_intents table exists with proper status CHECK constraint (8 values), UNIQUE constraints on stripe_payment_intent_id and idempotency_key, FK to transactions, full RLS policies, and comments.
  </done>
</task>
