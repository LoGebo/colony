...
CREATE POLICY violations_select_staff ON violations
  ...
  USING (
    community_id = (SELECT get_current_community_id())
    AND deleted_at IS NULL
    AND (SELECT get_current_user_role()) IN ('admin', 'manager', 'staff', 'guard')
  );
...
-- ============================================
-- Note: audit.enable_tracking adds triggers for change logging
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'enable_tracking' AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'audit')) THEN
    PERFORM audit.enable_tracking('public.violation_types'::regclass);
    PERFORM audit.enable_tracking('public.violations'::regclass);
  END IF;
END $$;
